...
(function () {
  ...
  // Always dark mode — clear any stale light preference
...
(function () {
  ...
  // Ensure no-results is hidden on init
  if (noResults) noResults.classList.remove('show');
  // Clear any browser-autofilled value
...
function showError(msg) {
  ...
  card.innerHTML = `
    <div class="result-header">
      ...
      <div><div class="result-title">Error</div><div class="result-sub">${escHtml(msg)}</div></div>
    </div>`;
  ...
  card.innerHTML = `
    <div class="result-inner">
      <div class="result-header">
        ...
        <div>
          <div class="result-title">File Ready!</div>
          <div class="result-sub">${escHtml(data.filename || '')}</div>
        </div>
      </div>
      ${extraHtml || ''}
    </div>
    ...
    <div class="result-footer">
      <a href="${escHtml(data.download_url)}" class="btn-download-new" download="${escHtml(data.filename)}">
        <i class="fa-solid fa-download"></i> Download File
      </a>
      <button class="btn-new-file" onclick="window.location.reload()">
...
function showToast(msg, type = 'success', duration = 3000) {
  ...
  toast.innerHTML = `<i class="fa-solid ${type === 'success' ? 'fa-check' : type === 'error' ? 'fa-xmark' : 'fa-info'}"></i> ${escHtml(msg)}`;
